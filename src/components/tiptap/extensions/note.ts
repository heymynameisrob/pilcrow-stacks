...
export const NoteExtension = Mark.create<NoteOptions, NoteStorage>({
  ...
  onSelectionUpdate() {
    ...
    if (!marks.length) {
      this.storage.activeNoteId = null;
      this.options.onNoteActivated(this.storage.activeNoteId);
      return;
    }

    const noteMark = this.editor.schema.marks.note;

    const activeNoteMark = marks.find((mark) => mark.type === noteMark);

    this.storage.activeNoteId = activeNoteMark?.attrs.noteId || null;
